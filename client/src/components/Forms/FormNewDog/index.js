...
import { newDog } from '../../../services/dogService.js';
...
class FormNewDog extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dog: {
    ...
    createDog = async () => {
        const response = await newDog(this.state.dog);
        const data = await response.json();

        if (response.ok) {
            this.props.closeNewDogModal();
    ...
    render() {
        ...
        return (
            <div>
                <form id="form-new-dog" noValidate autoComplete="off" onSubmit={this.handleSubmit}>
                    <TextField
                        required
                    ...
                    {this.state.dog.listVaccines.map((vaccine, index) => (
                        <div className={classes.vaccineFieldWrap} key={index}>
                            ...
                            {this.state.dog.listVaccines.length > 1 ?
                                <Fab onClick={this.handleVaccineDelete(index)} className={`${classes.btnDeleteVaccine} btn-delete-circle`} size="small" aria-label="delete">
                                    <DeleteIcon fontSize="small" />
                                </Fab>
                                : null}
                        </div>
                    ))}
                    <Fab className={`${classes.btnAddVaccine} btn`} onClick={this.addVaccine} disabled={!this.state.dog.listVaccines.every(value => value.trim() !== '')} size="small" aria-label="add">
                        <AddIcon />
                    </Fab>
                </form>
                <div className={classes.modalActionsForm}>
                    <Button
                        ...
                        onClick={this.props.closeNewDogModal}
                        size="large"
                    >
                        Cancel
                        </Button>
                    <Button
                        ...
                        size="large"
                        type="submit"
                        className={`${classes.btnSaveDog} btn`}
                        startIcon={<SaveIcon />}
                        form="form-new-dog"
                    >
                        Save
...
export default withStyles(useStyles)(FormNewDog);